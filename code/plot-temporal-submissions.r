...
gisaid_temporal_subs = main_df %>% group_by(wy) %>% summarise(sum_gisaid = sum(GISAID.weekly.submissions)) %>% arrange(wy)
cd19dp_temporal_subs = main_df %>% group_by(wy) %>% summarise(sum_cd19dp = sum(C19DP.weekly.submissions)) %>% arrange(wy)

gisaid_temporal_subs_av = main_df %>% group_by(wy) %>% summarise(av_gisaid = mean(GISAID.weekly.submissions)) %>% arrange(wy)
cd19dp_temporal_subs_av = main_df %>% group_by(wy) %>% summarise(av_cd19dp = mean(C19DP.weekly.submissions)) %>% arrange(wy)
